  ...
  local({
    ...
    observeEvent(eval(parse(text = vt.trigger)), {
      ...
      if (is.null(vt.param_area_ind)) {
        ...
        spldf.cyar_map <- spldf.cyar_map[spldf.cyar_map@data$code %in% df.tmp_map_attr$AREA_CODE_NO, ]
...
output$cyar_map_download <- downloadHandler(
  filename = function() {
    paste0("CYAR_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  ...
  content = function(file) {
    spdf <- cyar_panel$map
    df = spdf@data %>%
      select(YEAR, AREA_CODE, AREA_TYPE, AGE, GENDER, RISK_DESC, COUNT, TOTAL) %>% 
      mutate(PERC = COUNT/TOTAL) %>% 
      mutate(TOTAL = ifelse(TOTAL == 0, "s", as.character(TOTAL)),
             COUNT = ifelse(COUNT == 0, "s", as.character(COUNT)),
             PERC = ifelse(COUNT == "s" | TOTAL == "s", "s", percent(PERC))) %>% 
      change_names(df.cyar_csv_rename$ORIGINAL,
                   df.cyar_csv_rename$RENAMED)
    
    write.csv(df, file, row.names = F) 
  }
)

output$cyar_map_all_area_download <- downloadHandler(
  filename = function() {
    paste0("CYAR_all_area_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  },
  content = function(file) {
    df <- cyar_panel$map_filtered_data
    if(is.null(cyar_panel$param_area_ind)){
      vt.param_area_filter <- "REGION"
    } else{
      vt.param_area_filter <- cyar_panel$map_filtered_data %>% 
        filter(AREA_CODE %in% cyar_panel$param_area_ind) %>% 
        .[["AREA_TYPE"]] %>% 
        unique() %>% 
        toupper()
    ...
    vt.param_gender_filter <- unique(cyar_panel$map$GENDER)
    
    df = df %>%
      filter(toupper(AREA_TYPE) == vt.param_area_filter) %>% 
      filter(GENDER %in% vt.param_gender_filter) %>% 
