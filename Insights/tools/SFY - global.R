...
df.tmp_sfy_map <- df.db_sfy_sp %>% 
  ...
  filter(OTHER_IND == vt.init_sfy_param_other_ind) %>% 
  filter(PERIOD == vt.init_sfy_param_period) %>% 
  select(AREA_CODE, AREA_TYPE, GENDER, AGE, SERV_TYPE, SERV_IND, OTHER_TYPE, OTHER_IND, PERIOD, YEAR,
         RISK_DESC, METRIC_TYPE, COUNT)
...
df.tmp_sfy_map_total_people <- df.tmp_sfy_map %>% 
  ...
  group_by(AREA_CODE, AREA_TYPE, GENDER, AGE, RISK_DESC, OTHER_TYPE, OTHER_IND, PERIOD, YEAR) %>%
  summarise(TOTAL_RISK = sum(COUNT))

df.tmp_sfy_map_total_serv <- df.tmp_sfy_map %>% 
  filter(RISK_DESC %in% vt.risk_grps) %>% 
  group_by(AREA_CODE, AREA_TYPE, GENDER, AGE, SERV_IND, OTHER_TYPE, OTHER_IND, PERIOD, YEAR) %>%
  summarise(TOTAL_SERV = sum(COUNT))

df.init_sfy_map_data <- df.tmp_sfy_map %>% 
  left_join(df.tmp_sfy_map_total_serv, 
            by = c("AREA_CODE", "AREA_TYPE", "GENDER", "SERV_IND", "AGE", "OTHER_TYPE", "OTHER_IND", "PERIOD", "YEAR")) %>% 
  left_join(df.tmp_sfy_map_total_people,
            by = c("AREA_CODE", "AREA_TYPE", "GENDER", "RISK_DESC", "AGE", "OTHER_TYPE", "OTHER_IND", "PERIOD", "YEAR")) %>% 
...
#Mapping DF for renaming columns in csv download
df.sfy_csv_rename <- data_frame(ORIGINAL = c("YEAR", "PERIOD",	"AREA_CODE",	"AREA_TYPE",	"AGE", "GENDER",
                                             "SERV_TYPE", "SERV_IND", "RISK_DESC", 
                                             "COUNT", "TOTAL_SERV", "TOTAL_RISK", "PC_RISK", "PC_SERV"),
                                RENAMED = c("Year of interest", "Period of interest", "Area",	"Area type", "Age group",	"Gender",	
                                            "Service type", "Service", "Risk group",
                                            "Total service users in risk group", "Total service users", "Total risk group", "% of risk group using the service", "% of service users in risk group"))
