    ...
    observeEvent(eval(parse(text = vt.trigger)), {
      ...
      if (is.null(vt.param_area_ind)) {
        ...
        df.tmp_map_attr <- df.db_au_mapping %>% 
          change_names(c(vt.param_zoom_ind_curr, vt.param_zoom_ind, paste0(vt.param_zoom_ind, "_CODE_NO")), 
                       c("AREA_CURR", "AREA_ZOOM", "AREA_ZOOM_NO"), 
                       reminder = FALSE) %>% 
          filter(AREA_CURR %in% vt.param_area_ind) %>% 
          select(AREA_CURR, AREA_ZOOM, AREA_ZOOM_NO) %>% 
          unique() %>% 
          right_join(df.map_data, ., by = c("AREA_CODE" = "AREA_ZOOM", "AREA_CODE_NO" = "AREA_ZOOM_NO")) %>% 
        ...
        spldf.tyo_map <- spldf.tyo_map[spldf.tyo_map@data$code %in% df.tmp_map_attr$AREA_CODE_NO, ]
      }
      ...
      spldf.tyo_map@data <- spldf.tyo_map@data %>%
        left_join(df.tmp_map_attr, by = c("AREA_CODE", "code" = "AREA_CODE_NO")) %>%
        unique()
...
output$tyo_map_download <- downloadHandler(
  filename = function() {
    paste0("TYO_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  },
  content = function(file) {
    spdf <- tyo_panel$map
    df = spdf@data
    df = df %>% 
      mutate(TOTAL = PEOPLE/(PERCENT/100)) %>% 
      mutate(PERCENT = ifelse(PERCENT == 0, "s", percent(PERCENT/100)),
             PEOPLE = ifelse(PEOPLE == 0, "s", PEOPLE),
             TOTAL = ifelse(PEOPLE == 0 | PERCENT == 0, "s", round(TOTAL,2))) %>% 
      select(YEAR, AREA_CODE, AREA_TYPE, AGE, GENDER, RISK_FACTOR, TRAC_IND, PEOPLE, TOTAL, PERCENT) %>% 
      change_names(df.tyo_csv_rename$ORIGINAL,
                  df.tyo_csv_rename$RENAMED)
    
    write.csv(df, file, row.names = F) 
  }
)

output$tyo_map_all_area_download <- downloadHandler(
  filename = function() {
    paste0("TYO_all_area_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  },
  content = function(file) {
    df <- tyo_panel$map_filtered_data
    
    if(is.null(tyo_panel$param_area_ind)){
      vt.param_area_filter <- "REGION"
    } else{
      vt.param_area_filter <- df.db_tyo_sp %>% 
        filter(AREA_CODE %in% tyo_panel$param_area_ind) %>% 
        .[["AREA_TYPE"]] %>% 
        unique() %>% 
        toupper()
      
      vt.param_area_filter <- switch(vt.param_area_filter, "REGION" = "TA", "TA" = "AU", "AU" = "")
      }
    
    df <- df %>% 
      filter(toupper(AREA_TYPE) %in% vt.param_area_filter) %>% 
      filter(TRAC_IND %in% input$tyo_param_map_trac_ind) %>% 
      filter(AGE %in% input$tyo_param_map_age_group) %>% 
      filter(RISK_FACTOR %in% input$tyo_param_map_risk) %>%
      filter(GENDER %in% input$tyo_param_gender) %>%
      filter(YEAR %in% input$tyo_param_year) %>% 
      mutate(TOTAL = PEOPLE/(PERCENT/100)) %>% 
      mutate(PERCENT = ifelse(PERCENT == 0, "s", percent(PERCENT/100)),
             PEOPLE = ifelse(PEOPLE == 0, "s", PEOPLE),
             TOTAL = ifelse(PEOPLE == 0 | PERCENT == 0, "s", round(TOTAL,2))) %>% 
