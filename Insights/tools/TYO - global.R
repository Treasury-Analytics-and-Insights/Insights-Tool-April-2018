...
vt.tyo_col_blue3_d3 <- df.treasury_color %>%
  filter(colour == "blue6", degree == 3) %>%
  .[['code']]
...
vt.init_tyo_param_rg_list <- df.db_tyo %>% 
  select(RISK_FACTOR) %>% 
  distinct() %>% 
  mutate(RISK_NO = str_extract(RISK_FACTOR, "[0-9]+")) %>% 
  arrange(RISK_NO, RISK_FACTOR) %>% 
  group_by(RISK_NO) %>% 
  summarise(RISK_COMPARISON = paste(RISK_FACTOR, collapse = " / ")) %>% 
  filter(!is.na(RISK_NO)) %>% 
  .[["RISK_COMPARISON"]]

vt.init_tyo_param_rg <- vt.init_tyo_param_rg_list[1]
...
df.tyo_csv_rename <- data_frame(ORIGINAL = c("YEAR",	"AREA_CODE",	"AREA_TYPE",	"AGE",
                                              "GENDER",	"RISK_FACTOR", "TRAC_IND",	"PEOPLE",	"TOTAL", "PERCENT"),
                                 RENAMED = c("Year of interest", "Area",	"Area type",
                                             "Age group",	"Gender",	"Risk group", "Selected Outcome",
                                             "Person-years with selected outcome", "Total population person-years",
                                             "% of person-years with selected outcome"))
