...
observeEvent(input$sfy_param_refresh, {
  ...
  df.tmp_sfy_map <- df.db_sfy_sp %>%
    ...
    filter(OTHER_IND == vt.param_other_ind) %>%
    filter(PERIOD == vt.param_period) %>%
    select(AREA_CODE, AREA_TYPE, GENDER, AGE, PERIOD, YEAR,
           SERV_TYPE, SERV_IND, RISK_DESC, OTHER_IND, OTHER_TYPE,
           METRIC_TYPE, COUNT)
  ...
  df.tmp_sfy_map_total_people <- df.tmp_sfy_map %>%
    ...
    group_by(AREA_CODE, AREA_TYPE, GENDER, AGE, PERIOD, YEAR, OTHER_IND, RISK_DESC) %>%
    summarise(TOTAL_RISK = sum(COUNT))

  df.tmp_sfy_map_total_serv <- df.tmp_sfy_map %>%
    filter(RISK_DESC %in% vt.risk_grps) %>%
    group_by(AREA_CODE, AREA_TYPE, GENDER, AGE, PERIOD, YEAR, OTHER_IND, SERV_IND) %>%
    summarise(TOTAL_SERV = sum(COUNT))
  
  df.map_data <- df.tmp_sfy_map %>%
    left_join(df.tmp_sfy_map_total_serv,
              by = c("AREA_CODE", "AREA_TYPE", "GENDER", "AGE", "PERIOD", "YEAR", "OTHER_IND", "SERV_IND")) %>%
    left_join(df.tmp_sfy_map_total_people,
              by = c("AREA_CODE", "AREA_TYPE", "GENDER", "AGE", "PERIOD", "YEAR", "OTHER_IND", "RISK_DESC")) %>%
