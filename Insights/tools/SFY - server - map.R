...
output$sfy_map_download <- downloadHandler(
  filename = function() {
    paste0("SFY_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  },
  content = function(file) {
    
    df = sfy_panel$map@data %>% 
      inner_join(sfy_panel$map_data, by = "AREA_CODE") %>% 
      filter(RISK_DESC %in% sfy_panel$param_risk_ind, 
             SERV_IND %in% sfy_panel$param_serv_ind) %>% 
      select(YEAR, PERIOD, AREA_CODE,	AREA_TYPE, AGE, GENDER, 
             OTHER_IND, OTHER_TYPE, 
             SERV_TYPE,	SERV_IND,	RISK_DESC, 
             COUNT,	TOTAL_SERV,	TOTAL_RISK, PC_RISK,	PC_SERV) %>% 
      mutate(COUNT = ifelse(COUNT == 0, "s", as.character(round(COUNT,2))),
             TOTAL_SERV = ifelse(TOTAL_SERV == 0, "s", as.character(round(TOTAL_SERV,2))),
             TOTAL_RISK = ifelse(TOTAL_RISK == 0, "s", as.character(round(TOTAL_RISK,2))),
             PC_RISK = ifelse(PC_RISK == 0, "s", percent(PC_RISK/100)),
             PC_SERV = ifelse(PC_SERV == 0, "s", percent(PC_SERV/100)))
    
    tmp.OTHER_NAME <- unique(df$OTHER_TYPE)
    
    df = df %>% 
      change_names("OTHER_IND", tmp.OTHER_NAME) %>% 
      select(-OTHER_TYPE) %>% 
      change_names(df.sfy_csv_rename$ORIGINAL,
                   df.sfy_csv_rename$RENAMED)
    
    write.csv(df, file, row.names = F) 
  }
)

output$sfy_map_all_area_download <- downloadHandler(
  filename = function() {
    paste0("SFY_all_area_map_data_", format(Sys.time(), "%x_%H:%M"), ".csv")
  },
  content = function(file) {

    df <- sfy_panel$map_data
    
    if (is.null(sfy_panel$param_area_ind)) {
      vt.sfy_param_area_filter <- "Region"
    } else{
      vt.sfy_param_area_filter <- "TA"
    }
    
    df = df %>% 
      filter(AREA_TYPE %in% vt.sfy_param_area_filter, 
             RISK_DESC %in% sfy_panel$param_risk_ind, 
             SERV_IND %in% sfy_panel$param_serv_ind) %>% 
      select(YEAR, PERIOD, AREA_CODE,	AREA_TYPE,  AGE, GENDER,
             OTHER_IND, OTHER_TYPE, 
             SERV_TYPE,	SERV_IND,	RISK_DESC, 
             COUNT,	TOTAL_SERV,	TOTAL_RISK, PC_RISK,	PC_SERV) %>% 
      mutate(COUNT = ifelse(COUNT == 0, "s", as.character(round(COUNT,2))),
             TOTAL_SERV = ifelse(TOTAL_SERV == 0, "s", as.character(TOTAL_SERV)),
             TOTAL_RISK = ifelse(TOTAL_RISK == 0, "s", as.character(TOTAL_RISK)),
             PC_RISK = ifelse(PC_RISK == 0, "s", percent(PC_RISK/100)),
             PC_SERV = ifelse(PC_SERV == 0, "s", percent(PC_SERV/100)))
