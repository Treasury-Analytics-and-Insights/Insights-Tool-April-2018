...
output$tyo_facet_chart = renderPlotly({
  p = df.db_tyo %>%
    ...
    #If the Risk factor matches the risk factor grouping then include. 
    #Have to use .$RISK_FACTOR as this checkpoint is on dplyr 0.5
    #Note that this is compatible with dpylr 0.7
    mutate(RG_FILT = case_when(
      input$tyo_param_com_rg == "<2 Risk Indicators / 2+ Risk Indicators" & 
        (.$RISK_FACTOR %in% c("<2 Risk Indicators", "2+ Risk Indicators")) ~ T,
      input$tyo_param_com_rg == "<3 Risk Indicators / 3+ Risk Indicators" & 
        (.$RISK_FACTOR %in% c("<3 Risk Indicators", "3+ Risk Indicators")) ~ T,
      T ~ F
    )) %>% 
    filter(RG_FILT) %>%
    # mutate(RISK_FACTOR = factor(RISK_FACTOR, 
    #                             levels = c("<2 Risk Indicators", "2+ Risk Indicators", "<3 Risk Indicators", "3+ Risk Indicators"))
    #        )
    ...
    # mutate(TRAC_IND = replace(TRAC_IND, TRAC_IND %in% 'In Substantial Employment (with industry training)', 'In Substantial Employment \n(with industry training)')) %>%
    # mutate(TRAC_IND = replace(TRAC_IND, TRAC_IND %in% 'In Substantial Employment (without industry training)', 'In Substantial Employment \n(without industry training)')) %>%
