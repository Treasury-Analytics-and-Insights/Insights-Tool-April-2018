...
data project.idierp_mig(rename=(spell_stream=visa_type start_date=visa_start end_date=visa_end));
	merge idierp(in=a) %if &version.=archive %then %do; sanddol.migration_spells %end; 
			   %else %do; sanddol.migration_spells_&version. %end; (where=(start_date<="31Dec&year."d) keep=snz_uid spell_stream arrival_visa start_date end_date date_arrive date_depart);
	by snz_uid;
	if a;
...
proc sql;
   ...
   from data.address_notification_full
