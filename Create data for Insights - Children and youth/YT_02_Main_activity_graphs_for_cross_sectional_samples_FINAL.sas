...
merge 
  earners2011(keep=snz_uid earn8-earn12 rename=(earn8-earn12=earn149-earn153)) 
  ...
  earners2017(keep=snz_uid earn1-earn12 rename=(earn1-earn12=earn214-earn225));
by snz_uid;
...
else if 205<=i+&x.<=216 then minwage(i)=15.25;
else if 217<=i+&x. then minwage(i)=15.75;
end;
...
%transform_month_pop(project.population_2016_0_24,2016,197,218,22);
...
%run_mainact(project.population_2016_0_24,2016,22);
...
data YT_2016; merge project.YT_2016_TAB_YT_Outcomes_tool(in=a drop=pp_lt3_neet pp_st3_neet) inputlib._ind_ethnicity_&date.; by snz_uid; if a; 
if reg not in ('Area Outside Region'); run;

data TAB_combined; set YT_2013 YT_2014 YT_2015 YT_2016 ;run;
...
project.YT_2016_TAB_YT_Outcomes_tool(drop=pp_lt3_neet pp_st3_neet);
run;
...
data vars;
	set vars;
	if  type=1 /* need to also exclude any numeric variables here that you don't want to be rounded */;
	if name not in ("risk_factors_2plus_by15",'year');
call symput('numvars',_n_);
run;
...
proc export data=output2.YT_Table1_final_rr3 replace
outfile="&path.\tables\YT_REGION_final_2016.csv" dbms=csv;run;

proc export data=output2.YT_Table2_final_rr3 replace
outfile="&path.\tables\YT_TA_final_2016.csv" dbms=csv;run;

proc export data=output2.YT_Table3_final_rr3_  replace
outfile="&path.\tables\YT_AU_final_2016.csv" dbms=csv;run;
