...
proc format;
	value interv_grp
		...
		9='TRUENR'
		32='TRUATT'
		12,26,29,24,25,27,28,30,39='SEDU'
		10='EARLEX'
		11='HOMESCH'
		13,14='BOARD'
		16='RR'
...
** Add on the latest SNZ_UID to the historical interventions data (required as special education code 24 has been removed from the latest supply);
proc sql;
	create table interventions_old as
	select b.snz_uid, a.moe_inv_start_date, a.moe_inv_end_date, a.moe_inv_extrtn_date, a.moe_inv_intrvtn_code from
	sandmoe.interventions_20170420 a left join security.concordance b
	on a.snz_moe_uid = b.snz_moe_uid;
quit;

proc sql;
	create table interventionsb as select 
		a.snz_uid
		,input(compress(moe_inv_start_date,"-"),yymmdd10.) format date9. as startdate
		,input(compress(moe_inv_end_date,"-"),yymmdd10.) format date9. as enddate
		,input(compress(moe_inv_extrtn_date,"-"),yymmdd10.) format date9. as extractiondate
		,put(input(a.moe_inv_intrvtn_code,3.),interv_grp.) as interv_grp
		,b.DOB 
	from interventions_old a inner join pop_&year._0024_serv b 
		on a.snz_uid=b.snz_uid
	order by b.snz_uid;
quit;

data interventions;
	set interventions interventionsb;
...
data it_years(keep=snz_uid it_:);
	...
	by snz_uid;
	retain it_2009-it_2016;
	array it_yr{*} it_2009-it_2016;
	where startyr <= 2016 and endyr >= 2009;
	do y=1 to 7;
...
data tt(rename=(moe_ttr_year_nbr=year moe_ttr_training_prog_code=programme));
	...
	where 2009 le moe_ttr_year_nbr le 2016;
run;
...
data tt_years(drop=programme year y);
	set tt;
	by snz_uid;
	retain gway2_2009-gway2_2016 top_2009-top_2016 ygff2_2009-ygff2_2016 ytrain_2009-ytrain_2016;
	array gway2_yr{*} gway2_2009-gway2_2016;
	array top_yr{*} top_2009-top_2016;
	array ygff2_yr{*} ygff2_2009-ygff2_2016;
	array ytrain_yr{*} ytrain_2009-ytrain_2016;
	do y=1 to 7;
...
data sectert(rename=(moe_stp_enrolment_yr_nbr=year));
	...
	where 2009 le moe_stp_enrolment_yr_nbr le 2016;
run;
...
data st_years(keep=snz_uid st_:);
	set sectert;
	by snz_uid;
	retain st_2009-st_2016;
	array st_yr{*} st_2009-st_2016;
	do y=1 to 7;
...
proc sql;
	create table ygff as
		...
		WHERE 	fund_source='22' and moe_enr_efts_consumed_nbr>0 and moe_enr_qual_type_code="D" and
				year(input(moe_enr_prog_start_date,yymmdd10.))<=2016 and
...
data ygff_years(keep=snz_uid ygff_:);
	...
	by snz_uid;
	retain ygff_2009-ygff_2016;
	array ygff_yr{*} ygff_2009-ygff_2016;
	where startyr <= 2016 and endyr >= 2009;
	do y=1 to 7;
...
proc sql;
	...
	WHERE 2009 <= inc_cal_yr_year_nbr <= 2016 
		and  snz_ird_uid <> 0  
...
data sal_years(keep=snz_uid sal_:);
	set sal;
	by snz_uid;
	retain sal_2009-sal_2016;
	array sal_yr{*} sal_2009-sal_2016;
	do y=1 to 7;
...
data schools;
	merge pop_&year._0024_serv(in=a keep=snz_uid) inputlib._IND_SCH_ATTENDED_&date.;
	by snz_uid;
	if a;
run;
...
proc sql;
	...
	select a.*, b.schooltype as schooltype2006, 
				...
				l.schooltype as schooltype2016
	...
	left join all_schools k on a.school_in_2016=k.schoolnumber
	left join all_schools l on a.school_in_2016=l.schoolnumber
	order by snz_uid;
quit;
...
proc tabulate data=project.pop_&year._0024_service missing;
	...
	*tables (edu:)*age_desc*schter_1519,risk_factors_2plus_&year.*(N SUM)/NOCELLMERGE;
	tables (edu:),age_desc*SUM/NOCELLMERGE;
run;
proc tabulate data=project.pop_&year._0024_service missing;
